...
else{return this._setData(arguments[0],arguments[1]);}},destroy:function(){this._trigger('destroy');$('#demo-1').data('hygrid',false);},get:function(k,v){return this.options[k]||false;},set:function(k,v){return this.options[k]=v;},cols:function(visible){if(this.options.htmltable){var length=this._('tbody').find('tr:eq(0) td').length;}
else{var length=this._('thead').find('th.ui-hygrid-header').length;}
return this.options.cols>length&&this.options.length||length;},col:function(index,excludeHeader){var tbody=this._('tbody');var thead=this._('thead');return excludeHeader&&tbody.find('td:nth-child('+(index+1)+')')||thead.find('th:nth-child('+(index+1)+')').add(tbody.find('td:nth-child('+(index+1)+')'));},row:function(i){return this._('tbody').find('tr').eq(i);},cell:function(x,y,visible){var tbody=this._('tbody');return visible&&tbody.find('tr:visible').eq(y).find('td:visible').eq(x)||tbody.find('tr').eq(y).find('td').eq(x);},cells:function(visibleOnly){var tbody=this._('tbody');return visibleOnly&&tbody.find('td')||tbody.find('td:visible');},_setGridWidth:function(){var wrapper=this._('wrapper');var table=this._('table');switch(this.options.width||'auto'){case'auto':wrapper.width(table.width());break;case'fill':table.width(0);table.width(wrapper.width());break;default:wrapper.width(this.options.width);table.width(this.options.width);break;};},insertRow:function(cells){var row=$('<tr />');for(i in cells){var cell=this.options.cols&&this.options.cols[i]||{};cell.text=cells[i];cell.isTH=!(cell.isTD=true);cell.type='td';row.append(this._createCell(cell));}
